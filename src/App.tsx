...
import { getUsers , deleteUsers ,editUsers } from './service/API';
...
   }, [flag]);
  ...
  const handleEdit =  async (id :string,item: User | Role | Permission) => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'users':
        return <UserTable onEdit={handleEdit} onDelete={handleDelete} users={users} setFlag={setFlag}
        flag={flag} />;
      case 'roles':
